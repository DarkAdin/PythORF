try:
    Sequence=list(open(input('Insert exact filename of DNA sequence to be read: '),'r').read()) #DNA is read from a specific file. User is prompted to type the exact filename of the sequence, which must be in the same directory. FASTA format is not supported yet, since there needs to be a way of dealing with the first line that contains the metadata of the sequence. It will be implemented in time!
...
# ORF FINDER
############
def ORFinder(DNA): #Including everything in a function for the user to be able to call it.
    try:
    ...
    for i in range(3):
        ...
        Codon=[]
        Protein=[]
        for ribo in [Transcription[Nuc] for Nuc in [Compl[D] for D in DNA[Startpos:Endpos]][i:]]: #First, the DNA is turned into its complementary version. It is transcribed to RNA right after
            Count+=1
            Codon.append(ribo)
        ...
        while 'M' in Protein and '*' in Protein: #Making sure the protein contains methionins and stops.
            if Protein.index('M') < Protein.index('*'):
                if len(Protein[Protein.index('M'):Protein.index('*')])>int(ORFlength)/3: #The chosen minimal ORF Length comes into play. The length of all ORFs must obey its exact value, or higher. Else they are not considered.
...
ORFinder(Sequence)
if input('Find genomic targets (yes/no)? ') in ['No','no','NO','N','n']: #Finding targets should always be optional. Allowing the user to exit without doing anything else
    exit()
#########
# TARGETS
#########
def Targets(DNA):
    Count=0
    Targets=['ATG','CGGCGG','GCCGCC'] #List of targets to search for. It would be nice to make the user input all desired targets
